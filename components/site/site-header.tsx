...
export function SiteHeader() {
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-56 max-w-[1440px] items-center justify-between gap-6 px-6 md:px-12 lg:px-12 2xl:px-24">
        <Link
          href="/"
        ...
        <nav className="hidden items-center gap-3 lg:flex 2xl:gap-6">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="whitespace-nowrap text-[19px] uppercase tracking-wider opacity-90 transition hover:opacity-100 hover:text-gold 2xl:text-[21px]"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-4">
          ...
          <Link
            href="/contact"
            className={cn(
              "hidden rounded-full px-8 py-3.5 text-[19px] font-medium uppercase tracking-wider transition lg:inline-flex",
              scrolled
